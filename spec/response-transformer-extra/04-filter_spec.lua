...
for _, strategy in helpers.each_strategy() do
  describe("Plugin: response-transformer-extra (filter) [#" .. strategy .. "]", function()
    local proxy_client

    lazy_setup(function()
      local bp = helpers.get_db_utils(strategy, {"routes", "services", "plugins"}, {"response-transformer-extra"})
      ...
      assert(helpers.start_kong({
        ...
        plugins = "bundled,response-transformer-extra"
      }))
    end)
    ...
    describe("regressions", function()
      it("does not throw an error when request was short-circuited in access phase", function()
        -- basic-auth and response-transformer-extra applied to route makes request
        -- without credentials short-circuit before the response-transformer-extra
